function [ fit ] = fit_ACPGP( P, Y, regr, corr, lob, upb, theta0 )
% Normalized Periodic Gaussian Process Modeling 
% Inputs:
...
function [fit] = likelihood_at_decimal(para, fit) % likelihood at decimal
    ...
    for i = 1 : length(fit.P)
        ...
        if p>n
            ...
            B = SGG \ SGY; % beta
            sigma2 = (SYY - B'*SGG*B)/(k*p); % sigma
            fit.likelihood(i) = -(log(sigma2*delta^2) + sum(log(real(eig(Rdelta))))/(k*p) + 1 + log(2*pi))/2;
        end
...
function [obj, fit] = likelihood_at_interger(para, data) % likelihood at interger
    ...
    for i = 1 : length(data.intP)
        ...
        B = SGG \ SGY; % beta
        sigma2 = (SYY - B'*SGG*B)/(k*p); % sigma
        likelihood(p==data.P) = (log(sigma2*delta^2) + sum(log(real(eig(Rdelta))))/(k*p) + 1 + log(2*pi))/2;
