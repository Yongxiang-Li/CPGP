function out = testsmtoep(op1,op2,tau,m,n,show)
%TESTSMTOEP performs some tests on smtoep matrices.
%
%   TESTSMTOEP is called by VALIDATE.
%
%   See also VALIDATE.
...
if (nargin<5) || isempty(n),  n = 10;  end
if (nargin<4) || isempty(m),  m = 10;  end
if (nargin<3) || isempty(tau),  tau = 1e-8;  end

out = 0;

A = {op1(m,n) op2(m,n) op1(1,1) op2(1,1)};
B = {'A' 'B' 'a' 'b'};
...
% matmul
for i = 1:numel(matmul)
	for j = 1:4
	for k = 1:4
		strres = eval(['A{j}' matmul{i} 'A{k}''']);
		fulres = eval(['full(A{j})' matmul{i} 'full(A{k}'')']);
		err = full(strres-fulres);
		normerr = norm(err(:), inf) / norm(fulres(:), inf);
		str = sprintf('%s %s %s', B{j}, matmul{i}, B{k});
		if normerr > tau
			fprintf('%s\t\terr=%g\n', str, normerr)
			out = 1;
		elseif show
...
for i = 1:numel(mpow)
	for j = 1:2
	for k = 3:4
		strres = eval(['A{j}' mpow{i} 'round(abs(real(A{k})+2))']);
		fulres = eval(['full(A{j})' mpow{i} ...
				'full(round(abs(real(A{k})+2)))']);
		err = full(strres-fulres);
		normerr = norm(err(:), inf) / norm(fulres(:), inf);
...
for i = 1:numel(isfuns)
	for j = [1 3]
		switch isfuns{i}
		case 'isa'
			strres = eval([isfuns{i} '(A{j},''smtoep'')']);
			fulres = 1;
